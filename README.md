...
| PM | 모듈화 개발, 깃허브관리 | 피쳐분석 및 피쳐생성, Lightgbm, xgboost 모델개발 | Decision Tree 모델개발 | streamlit 기능구현, 서비스 개발 |
...
---
## 프로젝트 총평
  - 이상효: 저번 단위때는 아무생각 없었는데 팀장이 되보니 팀원 하나하나가 소중하다고 느꼈고, 모듈화를 진행하는 과정에서 다른사람들과의 코드 방식을 맞추면서 모듈화하기가 많이 힘들었는데, 모듈화가 성공적으로 되고 나서 딸깍을 해보니 모듈화를 왜 진행하는지 알 것 같았습니다. 마지막으로, 다들 팀장이나 팀원들이 어땠을지는 모르겠지만, 같이 협업하면서 좋은기억도 있을거고 나쁜기억도 있을텐데 좋은 기억들만 가져갔으면 좋겠습니다. 저 또한 좋은 기억들만 가져가겠습니다.

  - 황민우: 이번 프로젝트에서 저는 모델 제작과 모듈화 과정을 담당했습니다.
모델을 직접 구현하면서 기술적 이해를 넓혔고, 팀원들이 개발한 코드들을 약속된 구조로 통합하고 모듈로 재구성하는 역할을 맡으며 협업의 핵심을 체감할 수 있었습니다.
모듈화를 통해 단순한 코드 분리를 넘어 유지보수성과 확장성을 확보하는 것이 얼마나 중요한지 배웠습니다. 처음에는 낯설고 어렵게 느껴졌지만, 한 번 뼈대를 제대로 잡아두니 협업 효율이 크게 높아지고 유지보수 또한 수월해졌습니다.
이 경험을 통해 저는 코드 작성 규칙 확립과 원활한 의사소통이 팀 프로젝트 성공에 핵심적이라는 것을 깊이 깨달았습니다. 앞으로도 이러한 경험을 바탕으로 더 견고하고 확장 가능한 시스템을 설계하는 데 기여하고자 합니다.

  - 최은정: 이번 프로젝트를 통해 각자 다른 강점을 가진 팀원들과 협력하면서, 서비스 기획에 있어서 혼자였다면 놓쳤을 다양한 관점을 배우고 더 큰 성장을 경험할 수 있었습니다.
  
  - 채린: 이번 프로젝트를 통해 데이터 분석 과제의 전 주기를 직접 경험하며 데이터 엔지니어링과 머신러닝 운영을 아우르는 시야를 넓힐 수 있었습니다. 또한 주제에 맞게 데이터를 분석하고 주요 feature를 발굴해 이에 적합한 모델을 설계하는 과정의 중요성을 깨달았습니다. 더 나아가 분석 결과를 효과적으로 시각화하고 전달하는 방법에 대해 깊이 고민할 수 있었던 의미 있는 시간이었습니다.

  - 김창현: 이번 프로젝트를 통해 러신머닝의 분석 방법이 사람마다 다르다 라는 것을 배웠습니다. 더 나아가 더 생각하지 못한 방법들에 대해 느끼는 의미있는 시간이였습니다.
